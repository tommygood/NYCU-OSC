...
static void pool_refill(int pidx) {
    ...
    void *page = page_alloc_order(0); // require a page from order 0 (returns PA)
    if (!page) return;
    ...
    uint8_t *p = (uint8_t *)PA_TO_VA((uintptr_t)page);  // use VA for C access
...
int mm_init(const void *fdt) {
    ...
    /* Start bump pointer directly after the kernel image.
     * _end is a virtual address — convert to physical for the bump allocator. */
    uintptr_t after_kernel_pa = align_up(VA_TO_PA((uintptr_t)&_end), PAGE_SIZE);
    g_startup_cur = (uint8_t *)after_kernel_pa;
    g_startup_end = (uint8_t *)(uintptr_t)(mem_base + mem_size);
  
    // reserve memory for fdt (convert VA to PA)
    memory_reserve_startup(VA_TO_PA((uint64_t)(uintptr_t)fdt), 0x10000);
    ...
    void *frames_pa = startup_alloc(g_frame_count * sizeof(struct frame), PAGE_SIZE);
    if (!frames_pa) return -1;
    g_frames = (struct frame *)PA_TO_VA((uintptr_t)frames_pa);
    ...
    // reserve pages for kernel (convert VA to PA)
    memory_reserve(VA_TO_PA((uint64_t)(uintptr_t)&_start),
                   (uint64_t)((uintptr_t)&_end - (uintptr_t)&_start), "Kernel");

    // reserve pages for fdt (convert VA to PA)
    {
        uint32_t totalsize = bswap32(((const uint32_t *)fdt)[1]);
        if (totalsize) memory_reserve(VA_TO_PA((uint64_t)(uintptr_t)fdt), totalsize, "DTB");
    }
    ...
    // reserve pages for g_frames (convert VA to PA)
    memory_reserve(VA_TO_PA((uint64_t)(uintptr_t)g_frames),
                   g_frame_count * sizeof(struct frame), "Frame array");
...
// returns a VIRTUAL address (PA + PAGE_OFFSET)
void *allocate(size_t size) {
    ...
    if (size >= PAGE_SIZE || pidx < 0) {
        ...
        void *page = page_alloc_order(order);  // returns PA
        if (!page) return 0;
        return (void *)PA_TO_VA((uintptr_t)page);  // convert to VA
    }
    ...
    return (void *)c;  // pool chunks are already at VA (pool_refill uses VA)
}

// free page or chunk (ptr is a VIRTUAL address)
void free(void *ptr) {
    if (!ptr || !g_mm_ready) return;
    uintptr_t pa = VA_TO_PA((uintptr_t)ptr);
    ...
    if (pool < 0) {
        ...
        if (order >= 0) page_free_order((void *)pa, order);
        return;
    }
    ...
    uintptr_t page_base_va = PA_TO_VA(pa & ~(PAGE_SIZE - 1));
    uintptr_t page_base_pa = pa & ~(PAGE_SIZE - 1);
    size_t csz = g_pools[pool].chunk_size;
    size_t total = PAGE_SIZE / csz;
    size_t free_count = 0;
    struct chunk *cur = g_pools[pool].free_list;
    while (cur) {
        if (((uintptr_t)cur & ~(PAGE_SIZE - 1)) == page_base_va) free_count++;
        cur = cur->next;
    }
    if (free_count == total) {
        ...
        while (*pp) {
            if (((uintptr_t)*pp & ~(PAGE_SIZE - 1)) == page_base_va)
                *pp = (*pp)->next;
            else
                pp = &(*pp)->next;
        }
        g_frames[pidx].pool_idx = -1;
        page_free_order((void *)page_base_pa, 0);
    }
}
