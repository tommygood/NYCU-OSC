...
/*
 * Read PLIC size from /soc/plic or /soc/interrupt-controller reg property.
 * Returns 0 if not found.
 */
unsigned long dtb_get_plic_size(const void *fdt) {
    int soc_off = fdt_path_offset(fdt, "/soc");
    int addr_cells = (soc_off >= 0) ? fdt_address_cells(fdt, soc_off) : 2;
    int size_cells = 2;
    if (soc_off >= 0) {
        int len;
        const uint32_t *p = (const uint32_t *)fdt_getprop(fdt, soc_off, "#size-cells", &len);
        if (p && len == 4) size_cells = (int)bswap32(*p);
    }
    ...
    /* Size follows address in reg property */
    if (size_cells == 1) {
        if (len < (int)((addr_cells + 1) * 4)) return 0;
        return (unsigned long)bswap32(reg[addr_cells]);
    } else {
        if (len < (int)((addr_cells + 2) * 4)) return 0;
        return ((unsigned long)bswap32(reg[addr_cells]) << 32)
             | (unsigned long)bswap32(reg[addr_cells + 1]);
    }
}
