...
static void cmd_help(void) {
    uart_puts("Commands:\r\n"
              ...
              "  vfs_test      - test VFS (tmpfs create/write/read)\r\n");
}
...
static void cmd_vfs_test(void) {
    ...
    uart_puts("[vfs_test] create and write /hello.txt\r\n");
    if (vfs_open("/hello.txt", O_CREAT, &f) != 0) {
        uart_puts("  open(create) failed\r\n");
        return;
    }
    len = vfs_write(f, "Hello from tmpfs!", 17);
    uart_puts("  wrote "); uart_putdec((unsigned long)len); uart_puts(" bytes\r\n");
    vfs_close(f);

    uart_puts("[vfs_test] read back /hello.txt\r\n");
    if (vfs_open("/hello.txt", 0, &f) != 0) {
        uart_puts("  open(read) failed\r\n");
        return;
    }
    ...
    if (k_strcmp(buf, "Hello from tmpfs!") == 0)
        uart_puts("[vfs_test] PASSED\r\n");
    else
        uart_puts("[vfs_test] FAILED\r\n");

    uart_puts("[vfs_test] mkdir /mydir, create /mydir/test.txt\r\n");
    if (vfs_mkdir("/mydir") != 0) {
        uart_puts("  mkdir failed\r\n");
        return;
    }
    if (vfs_open("/mydir/test.txt", O_CREAT, &f) != 0) {
        uart_puts("  open(create in subdir) failed\r\n");
        return;
    }
    vfs_write(f, "nested file", 11);
    vfs_close(f);

    if (vfs_open("/mydir/test.txt", 0, &f) != 0) {
        uart_puts("  open(read in subdir) failed\r\n");
        return;
    }
    k_memset(buf, 0, sizeof(buf));
    len = vfs_read(f, buf, sizeof(buf) - 1);
...
static void dispatch(char *cmd, char *arg) {
    ...
    else if (k_strcmp(cmd, "vfs_test")   == 0) cmd_vfs_test();
    else if (cmd[0] != '\0') {
        uart_puts("Unknown: ");
        uart_puts(cmd);
        uart_puts("\r\n");
...
void kernel_main(void *fdt) {
    ...
    uart_puts("\r\n\r\n");
    uart_puts("OSC2026 Lab 7 - Virtual File System\r\n");
